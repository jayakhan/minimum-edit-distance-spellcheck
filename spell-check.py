"""This file uses google dictionary to replace corrupted words with the words with minimum edit distance"""
...
def compute_edit_distance(source_word, target_word):
    ...
    for a in range(1, n+1):
        for b in range(1, m+1):
            distance[a][b] = min(distance[a-1][b] + 1, (distance[a-1][b-1] + 0 if source_word[a-1]
                                 == target_word[b-1] else distance[a-1][b-1] + 2), distance[a][b-1] + 1)
    return distance[n][m]

# Return word from dictionary with min edit distance
...
if __name__ == '__main__':
    print(update_text_file('austen-sense-corrupted.txt'))
